...
public:
  DependencyGraph(const std::filesystem::path &directory_path, OpenMode mode = kLoadOrCreate,
    std::size_t memory_limit = kDefaultMemoryLimit, std::size_t chunk_bytes = kDefaultChunkBytes);
  ...
  void set_memory_limit(std::size_t memory_thresh) noexcept { memory_limit_ = memory_thresh; }
...
private:
  ...
  SymbolTable<ArchitectureType> &architectures() noexcept { return disk_graph_.architectures_; }
  const SymbolTable<ArchitectureType> &architectures() const noexcept { return disk_graph_.architectures_; }
  SymbolTable<DependencyType> &dependency_types() noexcept { return disk_graph_.dependency_types_; }
  const SymbolTable<DependencyType> &dependency_types() const noexcept { return disk_graph_.dependency_types_; }
  StringPool<false> &string_pool() noexcept { return disk_graph_.string_pool_; }
  const StringPool<false> &string_pool() const noexcept { return disk_graph_.string_pool_; }
